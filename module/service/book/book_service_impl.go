...
func (booksrv_imp *BookServiceImpl) HardDeleteBookSrv(ctx context.Context, bookId uint64) (book model.Book, err error) {
	_, err = booksrv_imp.FindByIdBookSoftDeleteSrv(ctx, bookId)
	if err != nil {
		return book, err
	}
...
func (booksrv_imp *BookServiceImpl) FindByIdBookSoftDeleteSrv(ctx context.Context, idIn uint64) (book model.Book, err error) {
	book, err = booksrv_imp.RepoBook.FindByIdBookSoftDelete(ctx, idIn)
	if err != nil {
		log.Printf("[ERROR] error findbook Book :%v\n", err)
		return book, err
	}
